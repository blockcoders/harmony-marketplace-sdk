...
describe('Bridge HRC721 Token', () => {
  ...
  before(async () => {
    ...
    hrc721 = new HRC721(hrc721Options.addr, hrc721Options.abi, WALLET_HMY_MASTER)
  ...
  describe('Send a HRC721 from Harmony to Ethereum network', () => {
    ...
    it(`hrc721 holder should have one token after mint`, async () => {
      const mintTx = await hrc721.mint(HMY_OWNER_ADDRESS, tokenId, E2E_TX_OPTIONS)
    ...
    it('Harmony Manager should lock the holder tokens', async () => {
      ...
      const lockTokenTx = await ownerHmyManager.lockNFT721Token(
        hrc721.address,
        tokenId,
        ETH_OWNER_ADDRESS,
        E2E_TX_OPTIONS,
      ...
      console.info('HRC721HmyManager lockNFT721Token tx hash: ', lockTokenTxHash)
  ...
  describe('Send an ERC721 (BridgedToken) from Ethereum to Harmony Network', () => {
    it('erc721 holder should approve Ethereum Manager', async () => {
      ...
      console.info('ERC721 (Bridged Token) approve tx hash: ', approveTx.transactionHash)
    })
